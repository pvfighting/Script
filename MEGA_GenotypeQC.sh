...
plink --bfile MEGAP60.GC025.ADHC --set-hh-missing --make-bed --out DATA.CleanSNPs
...
##Test data detected 467 problems since we do not assign the sex information##

####### Step3 remove SNP calling rate less than 95%
##Total genotyping rate is 0.995387.
##35716 variants removed due to missing genotype data (--geno).
##1730033 variants and 467 people pass filters and QC.
plink --bfile DATA.filterSubject --geno 0.05 --make-bed --out DATA.fileterSNPs 

####### Step4 Remove Hardy-Weinberg equilibrium SNPs
##--hwe: 21228 variants removed due to Hardy-Weinberg exact test.
##1708805 variants and 467 people pass filters and QC.
plink --bfile DATA.fileterSNPs --hwe 1e-6 --out DATA.Hardy --make-bed 

####### Step5 Remove Test-mishap SNPs
##1708325 variants and 467 people pass filters and QC.
...
plink --bfile DATA.Mis --maf 0.01 --out DATA.MAF --make-bed
#860358 variants removed due to minor allele threshold(s)
#(--maf/--max-maf/--mac/--max-mac).
#847967 variants and 467 people pass filters and QC.
...
sd = sd(data$F)
mean = mean(data$F)
data_normal = subset(data, F >= -4*sd); data_normal = subset(data_normal, F <= +4*sd)
...
# 6,736,714 variants and 335 people pass filters and QC.
...
perl /home/jp360/EIG-master/bin/smartpca.perl -i DATA.HC.QCed.longname.pruned.bed -a DATA.HC.QCed.longname.pruned.pedsnp -b DATA.HC.QCed.longname.pruned.pedind -o DATA.HC.QCed.longname.pruned.pca -p DATA.HC.QCed.longname.pruned.plot -e DATA.HC.QCed.longname.pruned.eval -l DATA.HC.QCed.longname.pruned.smartpca.log -m 5 -k 3 -t 2 
...
awk '/REMOVED/ {print $3}' DATA.HC.QCed.longname.pruned.smartpca.log | awk 'BEGIN{FS=":";OFS="\t"}{print $1,$2}' > DATA.HC.QCed.longname.pruned.smartpca.outlier.txt
plink --bfile DATA.HC.QCed --remove DATA.HC.QCed.longname.pruned.smartpca.outlier.txt --make-bed --out DATA.HC.QCed.Pass
