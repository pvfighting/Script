...
meta_eqtl_file = "/Users/jiajiepeng/workspace/p1/eqtl_data/Final.cis.eQTL.P1.xls"
data = read_tsv(meta_eqtl_file, col_names=F,  skip =1)
names(data)[1:6] = c("SNP","GENE","BETA","TSTAT","PVALUE","FDR")
...
#data$FDR_FE = p.adjust(data$PVALUE)
...
RE2_FDR = data %>% filter(FDR < 0.05) # 3 uniques gene?? 658 snps
...
Annotate <- function(df){
  ...
  by_gene$SNP_is_GWAS = SNP_anno$GWAS_SNP[match(by_gene$SNP_rs, SNP_anno$SNP)]
  by_gene$SNP_in_Region = SNP_anno$Region[match(by_gene$SNP_rs, SNP_anno$SNP)] 
...
############add the loci gene#########
#FE2_add = data %>%filter((GENE=="ENSG00000130203") |(GENE=="ENSG00000078487"))
#####################################
#meta_genes2 = Annotate(FE2_add)
#meta_egenes = rbind(meta_egenes,meta_genes2)
######################################
#RE2_all = data %>% filter(PVALUE_RE2 !=0) %>% select(GENE, SNP_rs, SNPPos, SNPChr, Region, PVALUE_RE2, FDR_RE2, STAT1_RE2, STAT2_RE2) %>% rename(SNP = SNP_rs, FDR=FDR_RE2) 
RE2_all = data %>% filter(PVALUE !=0)%>%filter(!((SNPChr==17)&(SNPPos >=56384022) )) #remove some variants which may not corrcet labeled. related with ABI3 loci in Chr 17
...
P_threshold = data_uniq %>% filter(FDR<0.05) %>% .$PVALUE %>% max
...
thin_data_uniq = data_uniq %>% mutate( logP = -log10(PVALUE) )
...
thin_data_uniq_down = thin_data_uniq_down %>% mutate(logP_scaled = (P_threshold_log)-offset + (logP - min(.$logP))/(max(.$logP)-min(.$logP)) * (offset) ) # normalize the -log10(pvalues) to (log2(P_threshold_log), log2(P_threshold_log)-offset)
thin_data_uniq_up = thin_data_uniq_up %>% mutate(logP_scaled = (logP))
...
pdf("/Users/jiajiepeng/workspace/p1/plot/Thin_manhattan.AD.cis.logscaled_4CSGrant23.pdf", width=5, height=2)
g = ggplot(thin_data_uniq, aes(x=newPos, logP_scaled))+ geom_point(aes(color=class), size=0.8, stroke=0) 
g = g + geom_hline(yintercept = (-log10(P_threshold)), color="red")
g = g + scale_x_continuous(breaks = the_lag$labelPos, labels = the_lag$SRegion, expand = c(0.025,0))
g = g + theme(axis.text.x = element_text(angle = 45, hjust = 1, size=4), axis.text.y = element_text(hjust = 1, size=4))
...
g = g + geom_text(data = the_lag, aes(x = labelPos, y = (P_threshold_log)-offset-0.3, label= Chr), size = 2, check_overlap = F, angle=0)
#g = g + geom_point(data = meta_egenes_top, aes(x = SNPnewPos, y = (-log10(PVALUE))), size=1.2, color = "goldenrod1",shape=18)
#g = g + geom_text(data = meta_egenes_top, aes(x = SNPnewPos, y = (-log10(PVALUE)), label = Gene_name), nudge_y=0.1, size = 1.5, check_overlap = F, angle= 45, hjust =0)
g = g + geom_point(data = meta_egenes, aes(x = SNPnewPos, y = (-log10(PVALUE))), size=1.2, color = "goldenrod1",shape=18)
g = g + geom_text(data = meta_egenes, aes(x = SNPnewPos, y = (-log10(PVALUE)), label = Gene_name), nudge_y=0.1, size = 1.5, check_overlap = F, angle= 45, hjust =0)

g = g + scale_colour_discrete(guide=F)
# g = g + xlab("Regions")
# g = g + ylab("-log10(Pvalue)")
g = g + scale_y_continuous(breaks = ( c(2, 4, 6) ), labels =  c(2, 4, 6), minor_breaks = (seq(1,8,1)), limits = c((P_threshold_log)-offset-0.35,(7)) )
